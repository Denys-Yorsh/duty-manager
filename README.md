# Модуль «Графік нарядів ВЧ V1.0.2»
...
![aplication screenshot](images/desctop.png)

---

### Інструкція зі встановлення та збирання модуля «Графік нарядів ВЧ V1.0.2»

  Цей проект базується на мові C++, фреймворку Qt 6 та базі даних SQLite.

  1. Підготовка середовища (Встановлення інструментів)

  Оскільки програма використовує графічний інтерфейс Qt, вам потрібно встановити відповідний набір інструментів:
   ...
   3. Встановлення компілятора:
       * Якщо ви вибрали MSVC, у вас повинна бути встановлена Visual Studio 2022 (з галочкою "Розробка класичних програм на C++").
  ...
  2. Налаштування проекту
   ...
   4. На екрані "Configure Project" виберіть свій "Kit" (наприклад, Desktop Qt 6.5.x MSVC2022 64bit) і натисніть Configure Project.
  ...
  3. Збирання та запуск
   ...
   3. При першому запуску програма автоматично створить файл бази даних duty_manager.db у папці збірки та ініціалізує всі таблиці.
  ...
  Щоб програма запускалася на іншому комп'ютері без встановленого Qt, виконайте наступне:
   ...
   5. Відкрийте термінал (PowerShell) та скористайтеся утилітою windeployqt, яка автоматично збере всі необхідні DLL:
   ...
   * Для перегляду бази даних вручну (якщо знадобиться) рекомендую програму DB Browser for SQLite.

  ---

  Примітка: Програма повністю автономна і працює без інтернету, що відповідає вимогам безпеки військової частини.
