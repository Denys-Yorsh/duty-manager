...
// Делегат для контролю відмальовування колонки Звання
...
public:
    ...
    void paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const override {
        if (index.column() == 1) {
            // Малюємо тільки фон комірки (щоб текст не дублювався під QComboBox)
    ...
    QWidget *createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const override {
        if (index.column() == 0 || index.column() == 4) return nullptr;
...
void PersonnelWidget::setupUi() {
    ...
    for (int i = 1; i < 5; ++i) m_view->horizontalHeader()->setSectionResizeMode(i, QHeaderView::Stretch);
...
void PersonnelWidget::manageStatuses() {
    QModelIndexList selected = m_view->selectionModel()->selectedRows();
    if (selected.isEmpty()) {
        QMessageBox::warning(this, "Попередження", "Оберіть в/c.");
        return;
    }
